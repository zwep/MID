...
i_id = 'ProstateX-0000'
i_re_type0 = pd_dat_img_ind.loc[i_id].index[7]
i_re_type = re.sub('[0-9]$', '',i_re_type0.replace('_',''))
pd_dat_img_ind.loc[(i_id, i_re_type0)]
A.loc[(i_id, i_re_type0)]

for i in os.walk(os.path.join(dir_img, i_id)):
    if i_re_type in i[0]:
        img_list = os.listdir(i[0])
        csv_img = [pydicom.read_file(os.path.join(i[0], x)) for x in img_list]
        break


z = [(i, np.array(i_img.ImagePositionPatient), i_img.SliceLocation) for i, i_img in enumerate(csv_img)]
w = [np.array(i_img.ImagePositionPatient) for i, i_img in enumerate(csv_img)]

sorted(z, key=lambda k: [k[1][2], k[1][1], k[1][0]])
